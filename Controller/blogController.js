const BlogModel = require("../Model/blogController");
...
const getblogwithauthor = async function (req, res) {
    ...
    try {
        ...
        let blogs = await BlogModel.find({ authorId: { $eq: authorId } }).populate("authorId") //$eq=equality condition
        console.log(blogs)
        if (blogs.length == 0) {
...
const updatedBlog = async function (req, res) {
    try {
        ...
        let updatedBlog = await BlogModel.findOneAndUpdate(
            { _id: blogId },
            {
                $set: {       //replace the value of the field with the specified value
                    title: alldata.title,
                    body: alldata.body,
                ...
                $push: { tags: req.body.tags, subcategory: req.body.subcategory }, //adds the array field wd values as its element
            },
            { new: true }
        )
...
const deleteDocument = async function (req, res) {
    ...
    try {
        ...
        let deleteDoc = await BlogModel.find({ isDeleted: false }, data)
        .updateMany({
             $set:{ 
                isDeleted: true,
                deletedAt: moment().format()
